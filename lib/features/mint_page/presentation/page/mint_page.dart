...
import 'package:idchats_flutter/features/mint_page/domain/repository/request_types.dart';
import 'package:idchats_flutter/features/mint_page/presentation/widgets/top.dart';
import 'package:idchats_flutter/features/mint_page/presentation/widgets/bottom.dart';
...
class MintPage extends StatelessWidget {
  ...
  BlocProvider<MintPageBloc> buildBody(BuildContext context) {
    return BlocProvider(
        create: (_) => sl<MintPageBloc>(),
        child:
            BlocBuilder<MintPageBloc, MintPageState>(builder: (context, state) {
          if (state is MintPageInitial) {
            context.read<MintPageBloc>().add(MintPageEvent(
